...
namespace LmpClient.Systems.SettingsSys
{
    public class SettingsMessageHandler : SubSystem<SettingsSystem>, IMessageHandler
    {
        ...
        {
            if (!(msg.Data is SettingsBaseMsgData settingsData))
            {
                return;
            }

            switch (settingsData.SettingsMessageType)
            {
                case SettingsMessageType.Reply:
                    if (!(settingsData is SettingsReplyMsgData reply))
                    {
                        return;
                    }

                    if (SettingsCatalogJoinLatch.HasBufferedCatalog)
                    {
                        if (!SettingsCatalogJoinLatch.TryTakeBufferedCatalog(out var wireVersion, out var rows))
                        {
                            return;
                        }

                        var merged = SettingsCatalogJoinMerge.RunMerge(
                            replyArrivedFirst: false,
                            applyPersistentSyncCatalog: () => TryApplyPersistentSyncCatalog(wireVersion, rows),
                            applySettingsReply: () => ProcessSettingsReply(reply));
                        if (!merged)
                        {
                            return;
                        }

                        TryAdvanceToSettingsSynced();
                    }
                    else
                    {
                        SettingsCatalogJoinLatch.BufferReply(reply);
                    }

                    break;

                case SettingsMessageType.PersistentSyncCatalog:
                    if (!(settingsData is PersistentSyncCatalogMsgData catalog))
                    {
                        return;
                    }

                    if (SettingsCatalogJoinLatch.HasBufferedReply)
                    {
                        if (!SettingsCatalogJoinLatch.TryTakeBufferedReply(out var bufferedReply))
                        {
                            return;
                        }
                        ...
                        var merged = SettingsCatalogJoinMerge.RunMerge(
                            ...
                            applyPersistentSyncCatalog: () => TryApplyPersistentSyncCatalog(
                                catalog.PersistentSyncCatalogWireVersion,
                                catalog.PersistentSyncCatalogRows),
                            applySettingsReply: () => ProcessSettingsReply(bufferedReply));
                        if (!merged)
                        {
                            return;
                        }
                    ...
                    {
                        SettingsCatalogJoinLatch.BufferCatalog(
                            catalog.PersistentSyncCatalogWireVersion,
                            catalog.PersistentSyncCatalogRows);
                    }
        ...
        private static bool TryApplyPersistentSyncCatalog(byte wireVersion, PersistentSyncCatalogRowWire[] rows)
        {
            if (!PersistentSyncSystem.TryApplyPersistentSyncCatalog(
                    wireVersion,
                    rows ?? Array.Empty<PersistentSyncCatalogRowWire>(),
                    out var catalogFailure))
            {
                NetworkConnection.Disconnect(catalogFailure ?? "[KSPMP] Persistent sync catalog failed.");
                return false;
            }
        ...
        private static void TryAdvanceToSettingsSynced()
        {
            if (MainSystem.NetworkState != ClientState.SyncingSettings)
            {
                return;
            }
        ...
        private static bool ProcessSettingsReply(SettingsReplyMsgData msgData)
        {
