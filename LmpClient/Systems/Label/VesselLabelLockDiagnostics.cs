...
namespace LmpClient.Systems.Label
{
    ...
    public static class VesselLabelLockDiagnostics
    {
        ...
        public const string LogTag = "LMP_VESSEL_LABEL_HUD_DIAG";
        ...
        /// <summary>Stage string passed from <see cref="Harmony.VesselLabels_ProcessLabel"/> after LMP reapplies the owner line.</summary>
        ...
        public static void LogEnabledBanner()
        {
            ...
            LunaLog.LogError(
                ...
                " | turn_off=D8 in LMP Status when done");
        ...
        public static void LogVesselLabelHarmonyPatchOrder()
        {
            ...
            if (patches == null)
            {
                LunaLog.LogWarning("[LMP] " + LogTag + " | code=VESSEL_LABEL_HARMONY_POSTFIXES | postfixes=<none>");
                return;
            }
            ...
            sb.Append("[LMP] ").Append(LogTag).Append(" | code=VESSEL_LABEL_HARMONY_POSTFIXES | postfixes=");
        ...
        public static void OnLockListAppliedDiag()
        {
            ...
            LunaLog.LogWarning("[LMP] " + LogTag + " | code=LOCK_LIST_APPLIED | hint=flight labels refresh scheduled");
        }

        /// <summary>
        /// Runs in a Harmony postfix ordered <b>before</b> <see cref="Harmony.VesselLabels_ProcessLabel"/> (before LMP injects the owner line).
        /// </summary>
            ...
            string actualText)
        {
            ...
            sb.Append("[LMP] ").Append(LogTag).Append(" | code=").Append(code)
            ...
            switch (code)
            {
                case "FLIGHT_LABEL_MISMATCH":
                    sb.Append("Mismatch right after LMP ProcessLabel postfix; another mod may rewrite label.text later in the same frame (see STRIPPED_AFTER_LMP if that fires) or stock skipped owner line.");
                    break;
                case "FLIGHT_LABEL_OWNER_STRIPPED_AFTER_LMP_HARMONY":
                    sb.Append("Owner line was present after LMP postfix this frame but missing at very-late Harmony postfix — strong evidence another mod runs after LMP on VesselLabels.ProcessLabel.");
                    break;
                case "OWNER_PREFIX_PRESENT_BEFORE_LMP_EVENT":
                    sb.Append("Label already had owner prefix before LMP postfix ran; unexpected ordering or double pipeline.");
                    break;
                ...
                case "STALE_FLIGHT_LABEL_BEFORE_LOCK_REFRESH":
                    sb.Append("Lock store already had owner but flight label text did not before LMP re-apply (late lock / missed refresh).");
                    break;
